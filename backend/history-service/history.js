...
appForHistory.post("/api/history/manage-code-attempt", async (req, res) => {
	...
	const { currUsername, matchedEmail, question, roomId, codeText, language } =
		req.body;
	...
	try {
		...
		if (codeAttemptExistsResult.rowCount > 0) {
			...
			return res.status(200).json({
				message:
					"Code attempt updated successfully into the database for storage.",
				data: req.body,
			});
		} else {
			// Else add the code attempt into the database as a new entry
			const insertQuery = `
			...
			return res.status(200).json({
				message:
					"Code attempt inserted successfully into the database for storage.",
			});
		}
	} catch (err) {
		console.error(err);
